...
| Method   | Endpoint                      | Frontend Function                 | Description                                                                                                                                                             |
| :------- | :---------------------------- | :-------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST`   | `/map/upload`                 | `uploadFile(file, tripId)`        | Uploads a file (image, GPX). Associates it with a `trip_id` query parameter if provided. Returns rich metadata (`UploadResponse` model). **Auth:** Only the trip owner or a listed member can upload. `uploader_id` is always set from the authenticated user (ignores client input). **Note:** For GPX files, this operation ensures a single GPX file per trip by replacing any existing one. |
| `GET`    | `/list-files/detail`          | `listImageFiles(tripId)`, `listGpxFilesWithMeta(tripId)` | Retrieves a detailed list of files (`FileListItem` model) for a given bucket and optional `trip_id` query parameter. Used to populate the image gallery and timeline. Required params: `bucket` (e.g., "images", "gps-data"). Includes a computed `can_delete` flag per item (true when requester is the trip owner or the original uploader). |
...
| `GET`    | `/gpx/{filename:path}/analysis` | `fetchGpxAnalysis(filename, tripId)` | Retrieves the analyzed data for a GPX track. Returns a JSON object containing: <br> - `coordinates`: List of `[lat, lon]` for the map polyline. <br> - `track_summary`: Stats like distance, duration, elevation gain, and `elevation_profile` (a list of `[distance, elevation]` points for graphing). <br> - `waypoints`: List of waypoints. <br> - `rest_points`: List of detected rest stops. <br> **Note:** If pre-calculated analysis is missing, the backend performs on-the-fly analysis to ensure data availability. |
| `DELETE` | `/map/delete/{filename:path}` | `deleteFile(filename, bucket)`, `deleteImage(filename)`, `deleteGpxFile(filename)` | Deletes a file from storage (MinIO) and its associated metadata document (MongoDB). Query param: `bucket` (default: "images"). Also removes any related analysis files for GPX tracks. **Auth:** Allowed only for the trip owner or the original uploader; otherwise returns 403/404. |
| `PATCH`  | `/photos/{metadata_id:path}/note` | `updatePhotoNote(id, noteData)` | Updates the `note` and `note_title` fields for a specific photo's metadata document. Request body: `{note: string, note_title: string}`. **Auth:** Trip owner or member can edit; if the file is orphaned (no trip_id), only the original uploader may edit. Used in the Timeline panel and ImageLayer popup. |
...
**Permission Model (files & notes):**
- Upload: trip owner or any listed member; server sets `uploader_id` to the authenticated user.
- Delete: trip owner or the original uploader; other members receive 403 even if they can view.
- Notes: trip owner or member may edit; orphaned files are editable only by the original uploader.
- Responses that list files include `can_delete` so the frontend can hide delete controls without trusting client-side checks.
