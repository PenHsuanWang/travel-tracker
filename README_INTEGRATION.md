...
| Method   | Endpoint                      | Frontend Function                 | Description                                                                                                                                                             |
| :------- | :---------------------------- | :-------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST`   | `/map/upload`                 | `uploadFile(file, tripId)`        | Uploads a file (image, GPX). Associates it with a `trip_id` query parameter if provided. Returns rich metadata (`UploadResponse` model). **Note:** For GPX files, this operation ensures a single GPX file per trip by replacing any existing one. |
